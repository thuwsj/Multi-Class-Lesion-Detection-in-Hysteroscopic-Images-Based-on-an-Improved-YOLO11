...
parser.add_argument('--weights', type=str, default=r'rtdetr-baseline.yaml', help='Path to model file.')
# 数据集存放路径
...
parser.add_argument('--name', type=str, default='rtdetr', help='Name data file.')
...
def train(model, data, epoch, batch, workers, device, name):
    model.train(data=data, epochs=epoch, batch=batch, workers=workers, device=device, name=name,optimizer=args.optimizer,seed=int(args.seed),single_cls=args.single_cls)
